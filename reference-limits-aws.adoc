...
* les types d'instances c4, m4 et r4 ne sont pas pris en charge avec les nouveaux systèmes Cloud Volumes ONTAP de cette version.
+
Toutefois, nous faisons toujours preuve des limites de disques pour ces types d'instances, car vous pouvez mettre à niveau un système vers cette version lorsqu'il est exécuté sur l'un de ces types d'instances.
...
[cols="26,26,48"]
|===
| Type de licence | Famille d'instances | Limite des machines virtuelles de stockage 


| *Freemium* | c5, m5 et r5  a| 
* 24 machines virtuelles de stockage total ^1,2^




| *PayGO basé sur la capacité ou BYOL* ^3^ | c5, m5 et r5  a| 
* 24 machines virtuelles de stockage total ^1,2^




.2+| *Facturation basée sur un nœud* | c4, m4 et r4  a| 
...
.2+| *BYOL sur nœud* ^4^ | c4, m4 et r4  a| 
* 1 VM de stockage pour l'accès aux données
* 1 VM de stockage pour la reprise après incident
